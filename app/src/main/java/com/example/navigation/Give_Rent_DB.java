...
public class Give_Rent_DB extends SQLiteOpenHelper {

    public static final String TABLE_NAME ="Give_Rent_Table";
    ...
    public Give_Rent_DB(@Nullable Context context) {
        super(context, "Give_Rent_Table", null, 1);
    }
    ...
    public Give_Rent_Note GetNote(int noteId){
        SQLiteDatabase db =this.getWritableDatabase();
        Give_Rent_Note note=null;
        Cursor cursor =null;
        try {
            cursor=db.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE " + " = " + ID + noteId ,null );
            if (cursor!=null){
                if (cursor.moveToFirst()){
                    note=new Give_Rent_Note(
