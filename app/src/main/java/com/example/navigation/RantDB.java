...
public class RantDB extends SQLiteOpenHelper {

    public static final String TABLE_NAME ="RantTable";
    ...
    public RantDB(@Nullable Context context) {
        super(context, "RantTable", null, 1);
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        String CREATE_TABLE="CREATE TABLE " + TABLE_NAME + "(" + ID + "INTEGER PRIMARY KEY AUTOINCREMENT, "
                + DAY + " INT, " + MONTH + " INT, " + YEAR + " INT, " + RENTER + " TEXT, " + SON_OF + " TEXT, " + MELI_NUMBER + " LONG, " + FROM_WHERE + " TEXT, " + MELI_CODE + " LONG, "
                + BIRTH_OF + " TEXT, " + ADDRESS + " TEXT, " + POST_CODE + " LONG, " + TELEPHONE + " LONG, " + MOBILE + " LONG)";
    ...
    public RentNote GetNote(int notId){
        SQLiteDatabase db =this.getWritableDatabase();
        RentNote note=null;
        Cursor cursor = null;
        try {
            cursor=db.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE " + ID + " = " + notId , null);
            if (cursor!= null){
                if (cursor.moveToFirst()){
                    note=new RentNote(
                            ...
                            cursor.getString(cursor.getColumnIndexOrThrow(RENTER)),
